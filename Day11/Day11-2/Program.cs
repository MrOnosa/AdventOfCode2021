...
int totalFlashes = 0;
int allFlash = -1;
for (int step = 1; allFlash < 0; step++)
{
    ...
    //Console.WriteLine("After energy "+(step ));
    //Print(dumbos);
    ...
    do
    {
       ...
       // Console.WriteLine("Flashing substep "+(step )+" Total falshes so far "+totalFlashes);
       // Print(dumbos);
    } while (dumbos.Any(r => r.Any(c => c.Energy > 9 && !c.Flashes)));

    if(dumbos.All(r => r.All(c => c.Flashes))){
        allFlash = step;
    }
...
Console.WriteLine($"Result: Step {allFlash} - Elapsed {stopWatch.Elapsed} ");
