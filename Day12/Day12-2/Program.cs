...
foreach (var path in start.Path)
{
    ...
    {
        if (p.Last() == "end" && p.Count(p => p == "end" || p == "start") == 2)
        {
            var duplicate = false;
...
List<List<string>> Explore(Cave cave, List<string> pathTaken)
{
    ...
    {
        var canExplore = false;
        if (path.IsBig)
        {
            canExplore = true;
        } else {

            var allSmallCaves = newPath.Where(p => p.All(c => char.IsLower(c))).ToList();
            var distinctSmallCaves = allSmallCaves.Distinct();
            if(allSmallCaves.Count() == distinctSmallCaves.Count()){
                //No small cave visted twice yet.
                canExplore = true;
            } else if(allSmallCaves.Count() == distinctSmallCaves.Count()+1){
                
                //Only one small cave visted twice yet.
                canExplore = true;
            }
        }
        if(canExplore)
